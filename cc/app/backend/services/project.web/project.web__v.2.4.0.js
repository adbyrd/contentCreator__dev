/**
 * [ FILE NAME : project.web.js : v2.4.0 ]
 *
 * Service: Project Service
 * Path: /backend/services/project.web.js
 * Version: [ PROJECT SERVICE : v.2.4.0 ]
 *
 * v2.4.0 — generateStoryboard Profile Enrichment
 * ─────────────────────────────────────────────────────────────────────────────
 * BUG: Five payload fields dispatched to n8n were blank on every execution:
 *      companyName, companyDescription, primaryCategory, customerType,
 *      targetAudience — causing INVALID_PAYLOAD rejection at Stage 1.
 *
 * ROOT CAUSE: These five fields are owned by the `profiles` collection, not
 *   the `projects` collection. generateStoryboard() only fetched the project
 *   record. The project record for this user contains empty strings for all
 *   five fields because they were never written there — they live on the
 *   user's profile. The payload assembly block used `project.<field> ?? ''`
 *   which correctly resolved the empty strings but had no fallback to the
 *   profile data source.
 *
 * CHANGES FROM v2.3.0:
 *   1. generateStoryboard() — new step 5a: fetch caller's profile record
 *      from the `profiles` collection in parallel with secrets retrieval.
 *      Profile fetch is non-blocking: if it fails or returns no record,
 *      execution continues and the project fields are used as-is. A warning
 *      is logged so the gap is visible in logs.
 *   2. Payload assembly — each of the five profile-sourced fields now uses a
 *      three-tier resolution chain:
 *        project field  →  profile field  →  empty string
 *      This means a value explicitly set on the project always wins, but if
 *      blank the profile value fills the gap automatically.
 *   3. Pre-dispatch field validation (step 5) now runs AFTER profile enrichment
 *      so it validates the final merged values, not the raw project fields.
 *      This surfaces a clear INCOMPLETE_PROFILE error if the profile itself
 *      is missing the required data, telling the user exactly where to go.
 *   4. COLLECTION_PROFILES constant added.
 *   5. All other exports are UNCHANGED from v2.3.0.
 *
...
const VERSION              = '[ PROJECT SERVICE : v.2.4.0 ]';
...
export const generateStoryboard = webMethod(Permissions.Anyone, async (projectId) => {
    ...
    try {
        ...
        // Resolution order per field: project value → profile value → ''
        // Project-level values win if present (future-proofing for per-project
        // overrides). Profile values fill the gap for the common case where
        // these fields are only set on the profile.
        const enriched = {
            ...
            targetAudience:     project.targetAudience     || profile?.targetAudience     || '',
        };

        console.log(`${VERSION} [${requestId}] Enriched fields — companyName: "${enriched.companyName}" | primaryCategory: "${enriched.primaryCategory}" | customerType: "${enriched.customerType}"`);

        // ── 6. Pre-dispatch field validation on merged values ─────────────────
        // Runs AFTER enrichment so it validates final resolved values.
        // If still blank after both project and profile have been checked,
        // the user needs to complete their profile settings.
        const missingFields = STORYBOARD_REQUIRED_FIELDS.filter((f) => !enriched[f]);

        if (missingFields.length > 0) {
            const fieldList = missingFields.join(', ');
            console.warn(`${VERSION} [${requestId}] Pre-dispatch validation failed after enrichment. Missing: ${fieldList}`);
            return {
                ok: false, status: 400,
                error: {
                    type:    'INCOMPLETE_PROFILE',
                    message: `Your profile is missing required fields: ${fieldList}. Please complete your profile settings before generating.`,
                },
            };
        }
