// [ FILE NAME : generateStoryboard.web.js : v1.4.0 ]
// Domain  : Storyboard
...
//
//   BUG FIXES
//   [FIX-01] Permissions.Member → Permissions.SiteMember (generateStoryboard was
//            already correct; cancelStoryboard was using Permissions.Member which
//            is not a valid Wix export — unified to SiteMember throughout).
//   [FIX-02] getMemberId: fieldset 'PUBLIC' does not expose _id on all Wix plans;
//            changed to fieldsets: ['FULL'] to guarantee _id resolution.
//   [FIX-03] postWithRetry: response.json() was called inside the .ok branch only.
//            On a retryable error the response body was never consumed, leaving the
//            connection open. Added explicit response.text() drain on non-ok paths.
//   [FIX-04] Payload assembly: `description` (project narrative field) was absent
//            from the n8n payload object. Added from project.description.
//   [FIX-05] Payload assembly: `secondaryCategory` is required by the n8n prompt
//            generation node (Part 1 spec §3.1) but was never included. Added from
//            profile.secondaryCategory with project-level override support.
//   [FIX-06] enriched merge: secondaryCategory added to the enriched context map
//            with the same resolution order (project → profile → '').
//   [FIX-07] REQUIRED_PROFILE_FIELDS updated to include 'secondaryCategory' so the
//            pre-dispatch validation gate catches a missing value before dispatch.
//
//   RELIABILITY
//   [REL-01] postWithRetry: AbortController timeout was set but the timer was only
//            cleared on success. On a retryable HTTP error path clearTimeout() was
//            called after the if-block, not before the continue, leaving a dangling
//            timer on short loops. Moved clearTimeout into a finally block to
//            guarantee cleanup on every code path.
//   [REL-02] postWithRetry: delay calculation used attempt - 1 as the exponent,
//            producing 0 ms on the first retry. Changed to `attempt` so delays are
//            500 ms → 1000 ms → 2000 ms (matches BASE_DELAY_MS intent).
//   [REL-03] rollbackStatus: spread operator on a large project object could fail
//            silently if wixData.update rejects due to a schema mismatch on
//            unrelated fields. Changed to a targeted patch using _id only.
//
//   OBSERVABILITY
//   [OBS-01] Added DISPATCH_COMPLETE log event after postWithRetry returns ok,
//            capturing submissionId + timestamp for end-to-end traceability.
//   [OBS-02] Enriched log now includes secondaryCategory.
//   [OBS-03] cancelStoryboard: no-op branch log level changed from log → warn so
//            repeated no-op cancel calls surface in error dashboards.
//
//   COMPLIANCE
//   [CMP-01] VERSION bumped to v1.4.0 across VERSION constant and file header.
//   [CMP-02] CANCEL_VERSION bumped to v1.2.0 to reflect the Permissions fix.
//   [CMP-03] debugWebhookStatus now returns CANCEL_VERSION alongside VERSION for
//            full component identity on debug inspection.
// ──────────────────────────────────────────────────────────────────────────────
...
const VERSION              = '[ GENERATE STORYBOARD : v1.4.0 ]';
...
// [FIX-07] secondaryCategory added — required by n8n prompt generation (Part 1 §3.1).
const REQUIRED_PROFILE_FIELDS = [
  'companyName',
  'companyDescription',
  'primaryCategory',
  'secondaryCategory',   // [FIX-07]
  'customerType',
];
 ...
 *         [FIX-04] description now included.
 *         [FIX-05] secondaryCategory now included.
...
export const generateStoryboard = webMethod(
  ...
  async (projectId) => {
    ...
    // secondaryCategory, and customerType. A null profile is not immediately
    ...
    //   secondaryCategory  → profiles collection  (Category Settings modal) [FIX-06]
    ...
    const enriched = {
      ...
      secondaryCategory:  project.secondaryCategory  || profile?.secondaryCategory  || '', // [FIX-06]
      customerType:       project.customerType       || profile?.customerType       || '',
      targetAudience:     project.targetAudience     || '',  // project-only field
    };

    // [OBS-02] secondaryCategory added to enriched log
    console.log(
    ...
    // [FIX-05] secondaryCategory added — required by n8n Part 1 prompt node.
    const n8nPayload = {
      ...
      primaryCategory:    enriched.primaryCategory,
      secondaryCategory:  enriched.secondaryCategory,    // [FIX-05]
      customerType:       enriched.customerType,
